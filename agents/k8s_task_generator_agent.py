...
def _get_generator_instructions():
    ...
    return (
        ...
        "4. session.json - Variables (format below)\n"
        "5. setup.template.yaml - Namespace + prerequisites\n"
        "6. answer.template.yaml - Complete solution\n"
        "7. test_01_setup.py - Deploy setup\n"
        "8. test_02_ready.py - Wait for setup resources (REQUIRED)\n"
        "9. test_03_answer.py - Deploy answer\n"
        "10. test_04_challenge.py - Optional: triggers/load\n"
        "11. test_05_check.py - Validate solution\n"
        "12. test_06_cleanup.py - Delete namespace\n"
        "\n"
        "=== concept.md ===\n"
        "Educational content that helps students learn the concept WITHOUT giving the answer.\n"
        "\n"
        "Structure:\n"
        "# Concept: [Resource Type]\n"
        "\n"
        "## What is it?\n"
        "- Brief explanation of the Kubernetes resource\n"
        "- Why it's useful\n"
        "- Common use cases\n"
        "\n"
        "## Key Components\n"
        "- Explain the main fields in the resource spec\n"
        "- What each field does\n"
        "- Common values and their meanings\n"
        "\n"
        "## Example Structure (Generic)\n"
        "```yaml\n"
        "# Show a GENERIC example with different names/values than the task\n"
        "# Use placeholder names like 'my-resource', 'example-app'\n"
        "# DO NOT use the actual variable names from session.json\n"
        "```\n"
        "\n"
        "## Useful Commands\n"
        "```bash\n"
        "# List relevant kubectl commands for working with this resource\n"
        ...
        "- Things to remember\n"
        "\n"
        "CRITICAL: concept.md must NOT contain:\n"
        "- The actual solution code\n"
        "- Variable names from session.json ({{namespace}}, {{pod_name}}, etc.)\n"
        "- Specific values required by the task\n"
        "- Copy-paste ready YAML that solves the challenge\n"
        "\n"
        "=== session.json ===\n"
        '{\n'
