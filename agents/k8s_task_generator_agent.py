...
def _get_generator_instructions():
    ...
    return (
        ...
        "# \n"
        "# CRITICAL: Use polling loops with time.sleep() to wait for resources to become ready\n"
        "# DO NOT just check once - resources take time to start!\n"
        "# \n"
        "# Decision process:\n"
        ...
        "    # ALWAYS use polling loops for pods, deployments, statefulsets, jobs\n"
        "    \n"
        "    def test_001_pod_ready(self, json_input):\n"
        "        # Example: If setup has a Pod - MUST use polling loop\n"
        ...
        "        # Example: If setup has a Deployment - MUST use polling loop\n"
        "        kube_config = build_kube_config(json_input['cert_file'], json_input['key_file'], json_input['host'])\n"
        "        max_wait = 60\n"
        "        interval = 2\n"
        "        \n"
        ...
        "            data = json.loads(result)\n"
        "            if data['status'].get('availableReplicas', 0) == data['spec']['replicas']:\n"
        ...
        "        # No polling needed - namespace is ready immediately\n"
        "        kube_config = build_kube_config(json_input['cert_file'], json_input['key_file'], json_input['host'])\n"
        ...
        "- Readiness checks should have reasonable timeouts (60-120 seconds)\n"
