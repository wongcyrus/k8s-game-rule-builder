...
def _get_fixer_instructions():
    ...
    return (
        ...
        "  - YAML syntax error in setup.template.yaml? → Fix only setup.template.yaml\n"
        ...
        "\n"
        "WHY THIS MATTERS:\n"
        "- Faster fixes (less file I/O)\n"
        "- Preserves working code\n"
        "- Easier to see what changed\n"
        "- Reduces risk of breaking working files\n"
        "\n"
        ...
        "3. concept.md - Learning material explaining the Kubernetes concept (NO solution code)\n"
        "4. session.json - Plain JSON with variables (NOT Jinja template)\n"
        "5. setup.template.yaml - Namespace + prerequisites (Jinja template)\n"
        "6. answer.template.yaml - Complete solution (Jinja template)\n"
        "7. test_01_setup.py - Deploy setup\n"
        "8. test_02_ready.py - Wait for setup resources (CRITICAL)\n"
        "9. test_03_answer.py - Deploy answer\n"
        "10. test_04_challenge.py - Optional: only if task needs triggers\n"
        "11. test_05_check.py - Validate solution\n"
        "12. test_06_cleanup.py - Delete namespace\n"
        "\n"
        "=== FIXING STRATEGY ===\n"
        "\n"
        "**Analyze First, Fix Second:**\n"
        "1. Read the error messages carefully\n"
        "2. Identify which specific file(s) are causing the problem\n"
        "3. Read those specific files\n"
        "4. Fix only what's broken\n"
        "5. Write only the fixed files\n"
        "\n"
        "**For Missing Files:**\n"
        "- Generate ONLY the missing file following the patterns from existing files\n"
        "- Use session.json variables consistently\n"
        "- Follow test patterns from similar tasks\n"
        "\n"
        "**For YAML Errors:**\n"
        "- Read and fix ONLY the broken YAML file (setup.template.yaml OR answer.template.yaml)\n"
        ...
        "- Write only the fixed YAML file\n"
        "\n"
        "**For Test Errors:**\n"
        "- Read the test output carefully to identify which test file is broken\n"
        "- Read ONLY that specific test file\n"
        ...
        "- Add proper timeout handling (60s max, 15s interval)\n"
        "- Write only the fixed test file\n"
        "\n"
        "**For Python Syntax Errors:**\n"
        "- Identify which .py file has the syntax error\n"
        "- Read and fix ONLY that file\n"
        "- Fix indentation issues\n"
        "- Add missing imports\n"
        "- Fix function signatures\n"
        "- Ensure proper pytest class structure\n"
        "- Write only the fixed Python file\n"
        "\n"
        "**Example Fix Workflow:**\n"
        "Error: \"test_02_ready.py: TimeoutError: Pod not ready in 60s\"\n"
        "1. Read session.json to see variables\n"
        "2. Read setup.template.yaml to see what resources exist\n"
        "3. Read test_02_ready.py to see what it's checking\n"
        "4. Identify the problem (e.g., wrong variable name)\n"
        "5. Fix ONLY test_02_ready.py\n"
        "6. Write ONLY test_02_ready.py back\n"
        "7. DO NOT write session.json, setup.template.yaml, or any other files!\n"
        "\n"
        ...
        "- DO NOT rewrite all files - only fix the broken ones!\n"
        "- READ ALL existing files first to understand the complete task structure\n"
        "- IDENTIFY which specific files are broken from the error messages\n"
        "- Make MINIMAL, TARGETED changes to fix specific errors\n"
        "- WRITE ONLY the files that you fixed\n"
        ...
        "- If a file is broken, fix it and write only that file\n"
        "- If a file is working, DO NOT write it - leave it unchanged\n"
    )
